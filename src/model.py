...
class CIRModel(nn.Module):
    def __init__(self, model_name="qihoo360/fg-clip-large", loss_weight=10.0):
    ...
    def preprocess_image(self, images):
        ...
        processed_image = torch.stack(processed_image).to(device)
    ...
    def encode_text(self, texts):
        if isinstance(texts, str):
            texts = [texts]
        input_ids = self.tokenizer(texts).to(device)
    ...
    def visual_out(self, x):
        ...
        x = torch.cat(
            [
                _expand_token(self.clip.visual.class_embedding, x.shape[0]).to(x.dtype),
                x,
            ],
            dim=1,
        )
    ...
    def extract_query(
        ...
        image_feature_cls, patch_feature = self.visual_out(x)
        ...
        reference_attention_map = 1 - (
            retained_attention_map - deleted_attention_map
        )  # (B,576)

        weighted_patch_feature = patch_feature * reference_attention_map.unsqueeze(
            2
        )  # (B,576,768)
        ...
        for i in range(B):
            ...
            retained_seg_score = (retained_seg_score - retained_seg_score.min()) / (
                retained_seg_score.max() - retained_seg_score.min() + 1e-2
            )
            ...
            deleted_seg_score = (deleted_seg_score - deleted_seg_score.min()) / (
                deleted_seg_score.max() - deleted_seg_score.min() + 1e-2
            )
    ...
    def compute_nce_loss(self, query_feature, target_feature):
        ...
        ground_truth = torch.arange(query_feature.shape[0], dtype=torch.long).to(device)
